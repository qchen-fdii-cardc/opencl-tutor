...
- `cl::Program(context, source)` / `program.build(...)`：编译内核。
- `cl::Kernel` / `kernel.setArg(...)`：准备内核调用。
- `cl::Buffer`：创建输入输出缓冲区。
- `queue.enqueueNDRangeKernel(...)` / `queue.enqueueReadBuffer(...)`：执行并读回。
